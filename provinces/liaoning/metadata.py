...
def infer_admission_category(title_or_filename: str) -> str | None:
    ...
    if any(k in text for k in ("艺术", "美术", "音乐", "舞蹈", "播音", "体育")):
        ...
        if "体育" in text:
            return "unsupported"
    if "普通类" in text or any(k in text for k in _ORDINARY_MARKERS):
        return "普通类"
    return None
...
def infer_subject_type(title_or_filename: str) -> str | None:
    text = title_or_filename or ""
    if "物理科目" in text or ("物理" in text and "历史" not in text):
        return "物理类"
    if "历史科目" in text or "历史" in text:
        return "历史类"
    return None
...
def infer_batch(title_or_filename: str, source_title: str | None = None) -> str:
    ...
    if "专科批" in text or "高职" in text or ("专科" in text and "本科" not in text):
        return "专科批"
